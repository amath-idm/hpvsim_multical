...
class MultiCal(sc.prettyobj):
    ...
    def __init__(self, sims, datafiles, common_pars=None, unique_pars=None,
                 fit_args=None, extra_sim_results=None, par_samplers=None,
                 n_trials=None, n_workers=None, total_trials=None, name=None,
                 db_name=None, load_if_exists=None, keep_db=None, storage=None,
                 rand_seed=None, label=None, die=False, verbose=True):
        ...
        self.run_args   = sc.objdict(n_trials=int(n_trials), n_workers=int(n_workers), name=name, db_name=db_name, keep_db=keep_db, storage=storage, rand_seed=rand_seed, load_if_exists=load_if_exists)
    ...
    def run_sim(self, sim, calib_pars=None, genotype_pars=None, label=None, return_sim=False):
                ...
                for gname, gpars in upars['genotype_pars'].items():
                    ...
                    for gpar, gval in gpars.items():
                        if isinstance(gval, dict):
                            this_genotype_pars[gpar] = dict()
                            for gparkey in gval.keys():
                                this_genotype_pars[gpar][gparkey] = trial_pars[f'{slabel}_{gname}_{gpar}_{gparkey}'] # Update with values from trial pars
                        else:
        ...
        for slabel,sim in self.sims.items():
            if self.unique_pars is not None:
                if self.unique_pars[slabel].get('calib_pars'):
                    calib_pars = sc.mergedicts(calib_pars, self.trial_to_sim_pars(self.unique_pars[slabel]['calib_pars'], trial, sname=slabel))
                if self.unique_pars[slabel].get('genotype_pars'):
                    for gname, pardict in self.unique_pars[slabel]['genotype_pars'].items():
                        ...
                        if genotype_pars.get(gname): # Some things have already been filled in from the common pars
                            genotype_pars[gname] = sc.mergedicts(genotype_pars[gname], unique_gpars) # NB, if something is in common pars and unique pars, it will be overwritten here with the unique pars values. TODO add check for this
                        else:
    ...
    def make_study(self):
        ...
        if self.run_args.rand_seed is not None:
            ...
            sampler = None
        output = op.create_study(storage=self.run_args.storage, study_name=self.run_args.name, sampler=sampler, load_if_exists=self.run_args.load_if_exists)
        return output


    def calibrate(self, common_pars=None, unique_pars=None, verbose=True, load=True, tidyup=True, **kwargs):
        '''
    ...
    def plot(self, slabel=None, res_to_plot=None, fig_args=None, axis_args=None, data_args=None, show_args=None, do_save=None,
        ...
        with hpo.with_style(**kwargs):
            ...
            for rn, resname in enumerate(age_results_keys):
                ...
                for date in all_dates[rn]:
                    ...
                    # Pull out data
                    thisdatadf = target_data[rn][(target_data[rn].year == float(date)) & (target_data[rn].name == resname)]
                    ...
                    if 'genotype' in resname:
                        for g in range(self.ng[slabel]):
                            ...
                            if glabel in unique_genotypes:
                                ...
                                ax.scatter(x, ydata, color=self.result_args[slabel][resname].color[g], marker='s', label=f'Data - {glabel}')
